...
import { useSaveSchedule } from '../hooks/use-schedule';
...
export default function DoctorSchedule({ doctorId, schedule }: DoctorScheduleProps) {
  ...
  const { saveSchedule, isPending, error } = useSaveSchedule();
  ...
  return (
    <div className="flex flex-col gap-5 justify-between flex-1">
      <div className="flex">
        <Button className="mt-4" onClick={save} disabled={isPending} variant="outline">
